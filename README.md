...
전체 구조:
데이터 정제 → KPI 정의 → 지표 계산 → 비교 분석 → 현상 리포트

1교시. 기술 분석의 역할과 범위

핵심 내용:
- 기술 분석은 “무슨 일이 발생했는가(What)”를 정량적으로 설명하는 단계
- 탐색 분석과의 차이:
  - 탐색: 문제 후보 찾기
  - 기술 분석: KPI로 현상 확정
...
- Markdown 제목도 허용합니다.
  - `# 1교시. 제목`
  - `# 📘 1교시. 제목`
- 첫 제목의 `(5교시 구성)` 같은 전체 설명은 교시로 인식하지 않습니다.
...
- 다음 형식의 메타 정보 줄은 `core_points`가 아니라 `meta`로 자동 분리됩니다.
  - 지원 라벨: `메모`, `참고`, `실습`, `주의`, `비고`, `목표`, `준비물`, `과제`, `핵심 구조`, `AI 활용`, `핵심 메시지`
  - `메모: 내용`
  - `참고: 내용`
  - `실습: 내용`
  - `핵심 구조: 내용`
...
- 라벨만 한 줄로 쓰고 다음 줄에 내용을 쓰는 방식도 허용합니다.
  ...
  - `이 장표는 실습 전에 설명`
- 교시 수만큼 PPT가 생성됩니다.
- `curriculum.md`에서 교시가 하나도 파싱되지 않으면, 프로그램은 교시별 생성이 아니라 단일 PPT 생성 경로로 동작할 수 있습니다.

### curriculum.md 생성을 위한 LLM 프롬프트 예시
아래 프롬프트를 그대로 LLM에 넣으면 `curriculum.md` 초안을 만드는 데 사용할 수 있습니다.

```text
너는 기업 교육 과정 설계자다.
아래 주제를 바탕으로 curriculum.md 형식의 강의 커리큘럼 초안을 작성하라.
...
3. 각 핵심 내용은 curriculum.md에 바로 넣을 수 있게 한 줄씩 작성하라.
4. 핵심 구조, AI 활용, 실습, 핵심 메시지, 목표가 필요하면 라벨 형식으로 작성하라.
...
출력은 설명 없이 curriculum.md 본문만 작성하라.
```
...
출력은 반드시 curriculum.md에서 바로 읽을 수 있는 Markdown/평문 형식으로 작성하라.
```
...
```powershell
.\run.ps1 -CurriculumFile .\prompts\another_curriculum.md
.\run.ps1 -CurriculumFile .\prompts\another_curriculum.md -lecture 1
```
...
- 같은 교시의 전체 `*_slide_plan.json`이 이미 있고 프롬프트/모델/Mock 여부가 같으면, 페이지 실행 시 해당 계획을 재사용해 LLM을 다시 호출하지 않습니다.
- 같은 교시의 전체 `*_slide_plan.json`, `*_llm_raw_response.txt`, `*_slide_plan.meta.json`이 있고 캐시 키가 일치하면 기본 실행에서도 해당 캐시를 우선 재사용합니다.
- 커리큘럼이나 프롬프트가 바뀌면 캐시 키가 달라져 새 slide plan을 생성합니다.
